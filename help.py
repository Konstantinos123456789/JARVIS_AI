"""
Helper functions for Jarvis AI — speech, system, web, and utilities.
"""

import os
import platform
import subprocess
from datetime import datetime

import psutil
import pyautogui
import pyttsx3
import requests
import speech_recognition as sr
import pywhatkit
import wikipedia

# Platform 
OS = platform.system()  # 'Windows' | 'Linux' | 'Darwin'

# Paths 
BASE_DIR = os.path.dirname(os.path.abspath(__file__))

# TTS Engine 
def _init_tts_engine() -> pyttsx3.Engine:
    try:
        if OS == 'Windows':
            engine = pyttsx3.init('sapi5')
        elif OS == 'Darwin':
            engine = pyttsx3.init('nsss')
        else:
            engine = pyttsx3.init('espeak')
        engine.setProperty('rate', 190)
        engine.setProperty('volume', 1.0)
        voices = engine.getProperty('voices')
        engine.setProperty('voice', voices[1].id if len(voices) > 1 else voices[0].id)
        return engine
    except Exception as e:
        print(f"TTS init error: {e}")
        return pyttsx3.init()

engine = _init_tts_engine()

# Small Talk
SMALL_TALK_PHRASES = {
    "how are you":        "I'm functioning as intended. How can I help you?",
    "what's the weather": "I don't have the ability to check the weather. Can I help with something else?",
    "hi":                 "Hello! How can I assist you today?",
    "hello":              "Hello! How can I help you?",
    "how is it going":    "I'm ready to assist. What do you need?",
    "goodbye":            "Goodbye! Have a great day!",
    "bye":                "Goodbye! Have a great day!",
    "what are you doing": "I am ready to help. What would you like me to do?",
    "i'm bored":          "I'm here to keep you entertained! What would you like to do?",
    "what can you do":    "I can answer questions, play music, open applications, and more.",
    "take a break":       "Sure, I'll be here when you're ready.",
    "i'm happy":          "That's great to hear! What's making you feel that way?",
    "lets get to work":   "Of course! What would you like to work on?",
    "thanks jarvis":      "You're welcome! I was made to be your assistant.",
    "thank you":          "You're welcome! Happy to help.",
    "whats your name":    "My name is Jarvis.",
    "who made you":       "I was created by my developer.",
    "are you there":      "Yes sir, I am here. How can I help you?",
}

# Core 
def speak(text: str) -> None:
    """Speak the given text using TTS."""
    try:
        engine.stop()
        engine.say(text)
        engine.runAndWait()
    except Exception as e:
        print(f"Speech error: {e}")


def take_user_input() -> str:
    """Listen and return the user's spoken input."""
    r = sr.Recognizer()
    with sr.Microphone() as source:
        print('Listening....')
        r.pause_threshold = 1
        audio = r.listen(source)
    try:
        print('Recognizing...')
        query = r.recognize_google(audio, language='en-in').lower()
        print(f"User said: {query}")
        for phrase, response in SMALL_TALK_PHRASES.items():
            if phrase in query:
                speak(response)
                return None
        return query
    except Exception:
        speak('Sorry, I could not understand. Could you please say that again?')
        return 'None'


def startup() -> None:
    """Run the Jarvis startup sequence."""
    messages = [
        "Initializing Jarvis",
        "Starting all systems applications",
        "Installing and checking all drivers",
        "Calibrating and examining all the core processors",
        "Checking the internet connection",
        "Wait a moment sir",
        "All drivers are up and running",
        "All systems have been activated",
        "Now I am online",
    ]
    for msg in messages:
        speak(msg)
        print(msg)


# System Info 
def cpu() -> None:
    """Report CPU and battery usage."""
    try:
        speak(f"CPU is at {psutil.cpu_percent()} percent")
        battery = psutil.sensors_battery()
        speak(f"Battery is at {battery.percent} percent" if battery else "Battery information not available")
    except Exception as e:
        print(f"Error getting CPU info: {e}")
        speak("Sorry, I couldn't get system information")


def screenshot() -> None:
    """Take and save a screenshot."""
    try:
        img = pyautogui.screenshot()
        folder = os.path.join(BASE_DIR, "Database", "Screenshots")
        os.makedirs(folder, exist_ok=True)
        img.save(os.path.join(folder, f"screenshot_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png"))
        speak("Screenshot saved successfully")
    except Exception as e:
        print(f"Screenshot error: {e}")
        speak("Sorry, I couldn't take a screenshot")


# App Launchers (cross-platform) 
def open_calculator() -> None:
    try:
        if OS == 'Windows':
            os.system('calc')
        elif OS == 'Darwin':
            subprocess.Popen(['open', '-a', 'Calculator'])
        else:
            subprocess.Popen(['gnome-calculator'])
        speak("Opening calculator")
    except Exception as e:
        speak("Sorry, I couldn't open the calculator")
        print(f"Error: {e}")


def open_camera() -> None:
    try:
        if OS == 'Windows':
            os.system('start microsoft.windows.camera:')
        elif OS == 'Darwin':
            subprocess.Popen(['open', '-a', 'FaceTime'])
        else:
            for app in ['guvcview', 'cheese', 'kamoso']:
                if subprocess.run(['which', app], capture_output=True).returncode == 0:
                    subprocess.Popen([app])
                    break
            else:
                speak("No camera app found. Install guvcview.")
                return
        speak("Opening camera")
    except Exception as e:
        speak("Sorry, I couldn't open the camera")
        print(f"Error: {e}")


def open_cmd() -> None:
    try:
        if OS == 'Windows':
            os.system('start cmd')
        elif OS == 'Darwin':
            subprocess.Popen(['open', '-a', 'Terminal'])
        else:
            subprocess.Popen(['gnome-terminal'])
        speak("Opening terminal")
    except Exception as e:
        speak("Sorry, I couldn't open the terminal")
        print(f"Error: {e}")


def open_notepad() -> None:
    try:
        if OS == 'Windows':
            os.system('notepad')
        elif OS == 'Darwin':
            subprocess.Popen(['open', '-a', 'TextEdit'])
        else:
            subprocess.Popen(['gedit'])
        speak("Opening text editor")
    except Exception as e:
        speak("Sorry, I couldn't open the text editor")
        print(f"Error: {e}")


# Web 
def play_on_youtube(video: str) -> None:
    try:
        speak(f"Playing {video} on YouTube")
        pywhatkit.playonyt(video)
    except Exception as e:
        speak("Sorry, I couldn't play that video")
        print(f"Error: {e}")


def search_on_google(query: str) -> None:
    try:
        speak(f"Searching Google for {query}")
        pywhatkit.search(query)
    except Exception as e:
        speak("Sorry, I couldn't perform the search")
        print(f"Error: {e}")


def search_on_wikipedia(query: str) -> str:
    try:
        return wikipedia.summary(query, sentences=2)
    except wikipedia.exceptions.DisambiguationError as e:
        return f"Multiple results found. Options include: {', '.join(e.options[:3])}"
    except wikipedia.exceptions.PageError:
        return "Sorry, I couldn't find that on Wikipedia"
    except Exception as e:
        print(f"Error: {e}")
        return "Sorry, there was an error searching Wikipedia"


def find_my_ip() -> str | None:
    try:
        ip = requests.get('https://api.ipify.org').text
        speak(f"Your IP address is {ip}")
        print(f"IP Address: {ip}")
        return ip
    except Exception as e:
        speak("Sorry, I couldn't find your IP address")
        print(f"Error: {e}")
        return None


# File Creation 
def create_file() -> None:
    """Create a new text file via voice input."""
    try:
        speak("What should be the name of the file?")
        filename = take_user_input()
        if not filename or filename == 'None':
            speak("I didn't catch that.")
            return

        folder = os.path.join(BASE_DIR, "Database", "Notepad")
        os.makedirs(folder, exist_ok=True)

        speak("What content do you want to write in the file?")
        content = take_user_input()
        if not content or content == 'None':
            speak("I didn't catch that.")
            return

        with open(os.path.join(folder, f"{filename}.txt"), "w") as f:
            f.write(content)
        speak(f"File {filename}.txt has been created.")
    except Exception as e:
        print(f"File creation error: {e}")
        speak("Sorry, I couldn't create the file")